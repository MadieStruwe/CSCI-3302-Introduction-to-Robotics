  ...
  switch(state){
    ...
    case STATE_HAS_PATH:
      ...
      if(goal_changed == TRUE)
      {
               if(path[index+1] != -1)
        {
          vertex_index_to_ij_coordinates(path[index], &next_i, &next_j);
          ij_coordinates_to_xy_coordinates(next_i, next_j, &next_x, &next_y);
          vertex_index_to_ij_coordinates(path[index], &theta_i, &theta_j);
          ij_coordinates_to_xy_coordinates(theta_i, theta_j, &theta_x, &theta_y);
          theta = atan2(theta_y-next_y, theta_x-next_x);
        }
        else
        {
          theta = 0;
    ...
    case STATE_SEEKING_POSE:
      ...
      {
        int next_i, next_j;
        float next_x, next_y, theta;
        theta = 0;
        updateOdometry();
        displayOdometry();
        vertex_index_to_ij_coordinates(index, &next_i, &next_j);
        ij_coordinates_to_xy_coordinates(next_i, next_j, &next_x, &next_y);
        set_pose_destination(next_x, next_y, theta);
        goal_changed = FALSE;
      }
